...
public:
	ControlSwitcherExpSynth(){
		...
		ControlGenerator phraseStart = ControlRandom().min(0).max(addParameter("phrasestart_max", 5).min(1).max(20)).trigger(modeSwitch);
		//ControlGenerator phraseLen = ControlRandom().min(3).max(11).trigger(modeSwitch);
		ControlGenerator phraseLen = ControlRandom().min(3).max(addParameter("phraselength_max", 11).min(4).max(30)).trigger(modeSwitch);
		...
		Generator bass =
			...
			>> LPF12().cutoff(addParameter("lpf_cutoff", 5500.0).min(50.0).max(10000.0)) //spi
			//>> LPF12().cutoff(11000)
